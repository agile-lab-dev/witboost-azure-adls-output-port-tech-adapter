...
- build the fat jar and launch the application with a JVM with the agent attached:
  ...
  java -javaagent:path/to/opentelemetry-javaagent.jar -jar common/target/adls-gen2-outputport-specific-provisioner.jar
  ```
...
OTEL_SERVICE_NAME=adls-gen2-outputport-specific-provisioner \
